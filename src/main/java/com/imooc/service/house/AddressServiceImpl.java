...
public class AddressServiceImpl implements IAddressService {
    ...
    private static final String BAIDU_MAP_KEY = "7a6h6NgouPW10khohEGOSj70OSKc8s46";
    ...
    public ServiceMultiResult findAllRegionsByCityName(String cityName) {
        ...
        List<SupportAddress> region = supportAddressRepository.findAllByLevelAndBelongTo(SupportAddress.Level.REGION.getValue(), cityName);

        for (SupportAddress supportAddress : region) {
            SupportAddressDTO target = modelMapper.map(supportAddress, SupportAddressDTO.class);
            addressDTOS.add(target);
        }
    ...
    public List<SubwayDTO> findAllSubwayByCity(String cityEnName) {
        ...
        subwayList.forEach(e -> result.add(modelMapper.map(e, SubwayDTO.class)));
        return result;
    }
    ...
    public List<SubwayStationDTO> findAllStationBySubway(Long subwayId) {
        ...
        subwayStationList.forEach(e -> result.add(modelMapper.map(e, SubwayStationDTO.class)));
        return result;
    }
    ...
    public ServiceResult lbsUpload(BaiduMapLocation location, String title, String address, Long houseId, int price, int area) {
        ...
        // 百度坐标系
        nvps.add(new BasicNameValuePair("geotable_id", "203924"));
        ...
        try {
            ...
            String result = EntityUtils.toString(response.getEntity(), "UTF-8");
            if (response.getStatusLine().getStatusCode() != HttpStatus.SC_OK) {
                log.error("Can not upload lbs data for response: {}", response);
                ...
                if (status != 0) {
                    ...
                    log.error("Error upload lbs data for status: {}, and message: {}", status, message);
                    return new ServiceResult(false, "Error upload lbs data");
                } else {
    ...
    private boolean isLbsDataExists(Long houseId) {
        ...
        sb.append("geotable_id=").append("203924").append("&")
        ...
        try {
            ...
            String result = EntityUtils.toString(response.getEntity(), "UTF-8");
            if (response.getStatusLine().getStatusCode() != HttpStatus.SC_OK) {
                log.error("Can not get lbs data for response: {}", response);
                return false;
            }
            JsonNode jsonNode = objectMapper.readTree(result);
            int status = jsonNode.get("status").asInt();
    ...
    public ServiceResult lbsRemove(Long houseId) {
        ...
        try {
            ...
            String result = EntityUtils.toString(response.getEntity(), "UTF-8");
            if (response.getStatusLine().getStatusCode() != HttpStatus.SC_OK) {
                log.error("Error delete lbs data for response: {}", response);
                return new ServiceResult(false);
            }
            JsonNode jsonNode = objectMapper.readTree(result);
            int status = jsonNode.get("status").asInt();
