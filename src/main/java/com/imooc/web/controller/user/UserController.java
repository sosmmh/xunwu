...
public class UserController {
    ...
    public ApiResponse subscribeList(@RequestParam(value = "start", defaultValue = "0") int start,
        ...
        ServiceMultiResult<Pair<HouseDTO, HouseSubscribeDTO>> serviceResult = houseService.querySubscribeList(HouseSubscribeStatus.of(status), start, size);
        if (serviceResult.getResultSize() == 0) {
            return ApiResponse.ofSuccess(serviceResult.getResult());
        }

        ApiResponse response = ApiResponse.ofSuccess(serviceResult.getResult());
        response.setMore(serviceResult.getTotal() > (start + size));
        return response;
    }
